    ...
    if (selector) {
      if (newPass.currentPassword && newPass.newPassword) {
        await changePassword(selector.id, newPass)
          ...
          .catch((err) =>
            setErrorPassMsg((prev) => (prev = err.response.data))
          );
      } else {
    ...
    try {
      if (selector) {
        await updateDetailsService(selector.id, newDetails)
          .then((res) => {
            toast.success("Your details have been successfully changed");
        ...
        <>
          ...
          <div className="allDetails">
            <div className="fullName">
              <h1>
                Hello {selector.firstName} {selector.lastName}
              </h1>
